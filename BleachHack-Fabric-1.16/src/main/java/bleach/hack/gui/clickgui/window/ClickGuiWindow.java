...
public abstract class ClickGuiWindow extends Window {
	...
	protected void drawBar(MatrixStack matrix, int mouseX, int mouseY, TextRenderer textRend) {
		...
		DrawableHelper.fill(matrix, x2 - 1, y1 + 1, x2, y2 - 1, 0xff8932a8);
		horizontalGradient(matrix, x1 + 1, y2 - 1, x2 - 1, y2, 0xff9a39bd, 0xff8932a8);

		DrawableHelper.fill(matrix, x1 + 1, y1 + 12, x2 - 1, y2 - 1, 0x90000000);

		/* title bar */
		horizontalGradient(matrix, x1 + 1, y1 + 1, x2 - 1, y1 + 12, 0xff9a39bd, 0xff8932a8);
